class PagesController < ApplicationController
    ...
    def search
        ...
        if (params[:start_date] && params[:end_date] && !params[:start_date].empty? & !params[:end_date].empty)
            ...
            @rooms.each do |room|
                not_available = room.reservations.where("(? <= start_date AND end_date <= ?) OR (? <= end_date AND end_date <= ?)
                OR (start_date < ? AND ? < end_date)", start_date, end_date, start_date, end_date, start_date, end_date).limit(1)
